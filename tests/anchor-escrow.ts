...
import { getAssociatedTokenAddressSync, createAssociatedTokenAccountInstruction, createMint, mintTo, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, getOrCreateAssociatedTokenAccount } from "@solana/spl-token";
...
describe("anchor_escrow", () => {
  ...
  before(async () => {
    ...
    await provider.connection.requestAirdrop(maker, 100 * anchor.web3.LAMPORTS_PER_SOL);
  ...
  it("Makes and takes the escrow", async () => {
    ...
    // Setup for take
    takerAtaA = (await getOrCreateAssociatedTokenAccount(provider.connection, taker, mintA, taker.publicKey)).address;
    makerAtaB = (await getOrCreateAssociatedTokenAccount(provider.connection, taker, mintB, maker)).address;
